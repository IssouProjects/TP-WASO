...
public class ServiceObjetMetier {
     ...
     public void rechercherClientParDenomination(String denomination, String ville)throws ServiceException{
         try {
            ...
            if(ville != null && !ville.isEmpty())
            {
                ...
                listeClients = this.dBConnection.launchQuery("SELECT ClientID, TypeClient, Denomination, Adresse, Ville FROM CLIENT WHERE (Denomination LIKE ? AND Ville=?) ORDER BY ClientID",denomination,ville);    
            }
            else
            {
    ...
    public void rechercherClientParNumero(Integer num) throws ServiceException {
        try {
            JsonArray jsonListe = new JsonArray();
            System.out.println("num:"+num);
            List<Object[]> listeClients = this.dBConnection.launchQuery("SELECT ClientID, TypeClient, Denomination, Adresse, Ville FROM CLIENT WHERE ClientID = ? ORDER BY ClientID", num);
