...
public class ServiceMetier {
    ...
    public void RechercherClientDenom(String denom, String ville) throws ServiceException, IOException {
        try {

            // 1. Obtenir la liste des Clients
            JsonObject clientContainer = this.jsonHttpClient.post(this.somClientUrl, new BasicNameValuePair("SOM", "rechercherClientParDenomination"), new BasicNameValuePair("denomination", denom), new BasicNameValuePair("SOM", "rechercherClientParDenomination"), new BasicNameValuePair("ville", ville));
            ...
            // récuperer les personnes correspondantes
            for (JsonElement client : jsonOutputClientListe) {
                JsonObject clt = client.getAsJsonObject();

                JsonArray personnesID = clt.get("personnes-ID").getAsJsonArray();

                String paramIDs = "";

                for (JsonElement personneID : personnesID) {
                    paramIDs += personneID.toString() + ";";
                }

                paramIDs = paramIDs.substring(0, paramIDs.length() - 1);
                System.out.println("param:"+paramIDs);
                JsonObject personneContainer = this.jsonHttpClient.post(this.somPersonneUrl, new BasicNameValuePair("SOM", "recupListePersonne"), new BasicNameValuePair("ListID", paramIDs));

                if (personneContainer == null) {
                    throw new ServiceException("Appel impossible au Service Personne::recupListePersonne [" + this.somPersonneUrl + "]");
                }

                clt.add("personnes", personneContainer.getAsJsonArray("personnes"));
    ...
    public void RechercherClientNumero(Integer num) throws ServiceException, IOException {
        try {

            // 1. Obtenir la liste des Clients
            JsonObject clientContainer = this.jsonHttpClient.post(this.somClientUrl, new BasicNameValuePair("SOM", "rechercherClientParNumero"), new BasicNameValuePair("numero", num.toString()));
